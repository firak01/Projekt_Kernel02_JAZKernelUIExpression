 ...
 *  So soll dann folgender Ausdruck aufgel�st werden:
...
public class KernelUIExpressionZZZ extends KernelUseObjectZZZ{
	...
	private void KernelExpressionUINew_(KernelJFrameCascadedZZZ frameParent, KernelJPanelCascadedZZZ panelDefault, String[] saFlag) throws ExceptionZZZ{
		main:{	
			 //setzen der übergebenen Flags	
			  ...
			  //Vereinfacht jetzt den ben�tigten JexlContext erstellen
			  this.jc = JexlHelper.createContext();	
			 	
		}//end main:
	}
	
	/**Berechnet den Ausdruck. In einem Ausdruck muss immer er PanelAlias stehen oder das Default Panel mu� im Constructor �bergeben worden sein !!!
	 *  Merke: Bei Verwendung dieser Methode gibt es auch keinen Alternativen DEFAUTL PANEL ALIAS
	 *  
	 * Beispiel f�r einen Ausdruck, der hiermit berechnet werden soll und aus einem Bedingungsteil und einem Ergebnisteil besteht
	 * 
	 ...
	 *                 Wird eine Berechnung zur�ckgegeben Vorname und Nachname.
	 *                 Ansonsten nur ein Leerstring.
	 *         
	 *         //MERKE: Eine Verschachtelung der Ausdr�cke ist momentan nicht erlaubt.
	 *		   //MERKE: Eine Formel im Bedingungsteil ist momentan nicht erlaubt. Es kann nur auf gef�llt/nicht gef�llt gepr�ft werden.
	 *		   //MERKE: Es ist nicht m�glich Parameter in den 'konfigurierten Methoden der Komponenten' zu �bergeben.
	...
	public String computeString(String sPanelAliasDefault, String sExpressionRaw) throws ExceptionZZZ{
		...
		main:{
			...
			if(saCondition.length>=2){			
				...
				//Es gibt keine Formel f�r den Bedingungsteil
...
//			1. Tokenizer mit den Aliasnamen erstellen. Trennzeichen "<+/>"
			//NEIN, der Tokenizer betrachtet nur einzelne Charkter-Zeichen als Delimiter    StringTokenizer tokenizer = new StringTokenizer(sExpressionPartRaw, "<+/>", false); //Der Delimiter selber soll nicht zur�ckgegeben werden
			String[] saExpressionPart = StringZZZ.explode(sExpressionPartRaw, "<+/>");
						
			
//			2. In einer Schleife die Werte der Aliasnamen durchgehen. 
			//        2a. Falls der Aliasname nicht existiert, wird der String komplett als konstante gesehen und �bernommen
...
//				++++++++    3. diesen Ausdruck ggf. noch in einen PanelAlias und einen FeldAlias zerlegen
				//Merke: Die Methoden einer Komponente m�ssen auch angegeben werden und werden nur durch einfachen Punkt '.' hinter den FeldAlias geh�ngt.
...
//					+++++++++++++++++++ 4a. Suchen der durch die Aliasse bestimmten Komponenten/Panels ++++++++++++++++++++++
					...
					//FALL: IM AUSDRUCK WURDE KEIN PANELNAME ANGEGEBEN. ERGO AUF EIN DEFAULT-PANEL ZUR�CKGREIFEN
...
//						Hier das Panel anhand des defaultPanelAliasses suchen.
					...
					if(!StringZZZ.isEmpty(sObject)  && panelCascaded != null){
						...
						//TODO: Merke: ggf. sind mehrere Methoden, die aufeinander folgen m�glich. Diese Aufsummierung fehlt noch.
						
						String sAliasComponent = tokenizermethod.nextToken();
						//4a. Die Komponente holen und JEXL verf�gbar machen
	...
	//					4ba. Die Formel erstellen, die dann durch JEXL aufgel�st werden kann
						...
						//4bb. Die Formel erstellen f�r KONSTANTEN String					
						if(StringZZZ.isEmpty(sFormulaFilled)){
